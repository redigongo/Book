...
import (
	...
	bookssvr "bookapp/gen/http/books/server"
	"context"
	"log"
	"net/http"
	"net/url"
...
func handleHTTPServer(ctx context.Context, u *url.URL, booksEndpoints *books.Endpoints, wg *sync.WaitGroup, errc chan error, logger *log.Logger, debug bool) {
	...
	)
	{
		...
		booksServer = bookssvr.New(booksEndpoints, mux, dec, enc, eh, nil, booksapi.BooksCreateDecoderFunc, booksapi.BooksUpdateDecoderFunc)
		if debug {
